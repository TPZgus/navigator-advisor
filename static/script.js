document.addEventListener('DOMContentLoaded', () => {
    ...
    let webSocket;
    let isMuted = false;
    let currentStream; // To hold the stream for stopping tracks
    let isServerReadyForAudio = false; // To ensure server is ready before recording
...
`;
    ...
    function sendTextMessage() {
        ...
        if (text && webSocket && webSocket.readyState === WebSocket.OPEN) {
            webSocket.send(JSON.stringify({ type: 'user_text_input', text: text }));
    ...
    async function connectWebSocket() {
        ...
        webSocket.onopen = () => {
            ...
            voiceOrb.textContent = 'Click to start recording voice message'; // Updated text
        ...
        webSocket.onmessage = (event) => {
            try {
                ...
                // if (message.type === 'text') { // Old handler for potentially partial AI text
                //     // This might be re-enabled if interim text updates are desired,
                //     // but for now, final_ai_text provides the complete response.
                //     // addMessageToConversation(message.data, 'ai'); 
                // } else
                if (message.type === 'final_ai_text') { // New handler for the complete AI text response
                    addMessageToConversation(message.data, 'ai');
                    // Ensure speaking class is removed if it was added for audio playback
                    if (voiceOrb.classList.contains('speaking')) {
                        voiceOrb.classList.remove('speaking');
                    }
                     // Update orb text if session is still connected and not recording
                    if (voiceOrb.classList.contains('connected') && !voiceOrb.classList.contains('recording')) {
                        voiceOrb.textContent = 'Click to start recording voice message';
                    }
                    ...
                    if (message.data === 'Gemini session initialized') {
                        isServerReadyForAudio = true;
                        console.log('[ClientState] Server is ready for audio.');
                        if (voiceOrb.classList.contains('connected') && !voiceOrb.classList.contains('recording')) {
                           voiceOrb.textContent = 'Click to start recording voice message'; // Updated text
                        }
                    } else if (message.data && message.data.startsWith('Gemini session with Google closed')) {
        ...
        webSocket.onclose = () => {
            ...
            voiceOrb.classList.remove('connected', 'recording', 'speaking');
            voiceOrb.textContent = 'Click to call your TPZ advisor!'; // Reverted text
            voiceOrb.style.cursor = 'pointer';
    ...
    async function startRecording() {
        if (!webSocket || webSocket.readyState !== WebSocket.OPEN) {
            addMessageToConversation('Not connected. Click orb to connect first.', 'system');
            return;
        }
        if (!isServerReadyForAudio) {
            addMessageToConversation('Session with advisor not fully ready. Please wait.', 'system');
            console.warn('[UserAudio] Attempted to record but isServerReadyForAudio is false.');
            return;
        }
        if (isMuted) {
            addMessageToConversation('Mic is muted. Unmute to record.', 'system');
            return;
        }
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            try {
                ...
                voiceOrb.classList.remove('speaking', 'connected');
                voiceOrb.textContent = 'Click to stop recording and send'; // Updated text
    ...
    voiceOrb.addEventListener('click', async () => {
        ...
        if (!webSocket || webSocket.readyState === WebSocket.CLOSED || webSocket.readyState === WebSocket.CLOSING) {
            console.log('[UserSession] Orb clicked: Initiating new session.');
            voiceOrb.textContent = 'Connecting...';
    ...
    muteButton.addEventListener('click', () => {
        ...
        muteButton.textContent = isMuted ? 'Unmute' : 'Mute Mic';
        console.log(`[UserAudio] Mic muted: ${isMuted}`);

        if (isMuted && mediaRecorder && mediaRecorder.state === "recording") {
            addMessageToConversation('Microphone Muted. Current recording will continue, but new recordings cannot start while muted.', 'system');
        } else if (!isMuted && mediaRecorder && mediaRecorder.state === "recording") {
            addMessageToConversation('Microphone Unmuted.', 'system');
        } else if (!isMuted && (!mediaRecorder || mediaRecorder.state === "inactive")) {
            addMessageToConversation('Microphone Unmuted.', 'system');
        } else if (isMuted && (!mediaRecorder || mediaRecorder.state === "inactive")) {
            addMessageToConversation('Microphone Muted.', 'system');
        }
    });

    initializeAudioSystem().then(() => {
        console.log("Initial audio system check complete.");
        voiceOrb.textContent = 'Click to call your TPZ advisor!'; // Initial text
        voiceOrb.style.cursor = 'pointer';
        sendTextButton.disabled = true;
        textInput.disabled = true;
        muteButton.disabled = true;
